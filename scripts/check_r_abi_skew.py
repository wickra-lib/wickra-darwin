...
takes the header and library from `WKDARWIN_INC` / `WKDARWIN_LIB`, supplied from
outside, and compiles the hand-written `src/wickra_darwin.c` against whatever
...
This makes the pairing explicit:
  ...
  * Every `wickra_*` symbol `src/wickra_darwin.c` calls must be declared in
    `bindings/c/include/wickra_darwin.h`. A call to a symbol that is not
...
downloads a prebuilt `wickra-darwin-c-<triple>.tar.gz` for the version in
...
HEADER = "bindings/c/include/wickra_darwin.h"
