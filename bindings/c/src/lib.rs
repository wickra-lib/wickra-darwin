//! The wickra-darwin C ABI — the hub every C-capable language links against.
//!
//! The surface is tiny and JSON-shaped, exactly like [`wickra_darwin_core::Darwin`]:
//! construct a handle from a spec JSON, drive it with command JSONs (`set_spec`,
//! `evolve`, `best`, `version`), read back response JSONs, and free the handle.
//! No darwin type crosses the boundary by value — the handle is opaque and the
...
//! ([`WICKRA_DARWIN_ERR_NULL`], [`WICKRA_DARWIN_ERR_UTF8`]) and caught panics
//! ([`WICKRA_DARWIN_ERR_PANIC`]); a non-negative return is always the response
...
pub const WICKRA_DARWIN_ERR_NULL: i32 = -1;
/// `cmd_json` was not valid UTF-8.
pub const WICKRA_DARWIN_ERR_UTF8: i32 = -2;
/// A panic was caught at the FFI boundary.
pub const WICKRA_DARWIN_ERR_PANIC: i32 = -3;
...
#[no_mangle]
pub unsafe extern "C" fn wickra_darwin_new(spec_json: *const c_char) -> *mut WickraDarwin {
    let result = catch_unwind(|| {
        let spec = unsafe { opt_str(spec_json) }?;
        Darwin::new(spec).ok().map(|inner| {
            Box::into_raw(Box::new(WickraDarwin {
                inner,
                pending: None,
            }))
        })
...
#[no_mangle]
pub unsafe extern "C" fn wickra_darwin_free(handle: *mut WickraDarwin) {
    if !handle.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_darwin_command(
    handle: *mut WickraDarwin,
    cmd_json: *const c_char,
    out: *mut c_char,
    cap: usize,
) -> i32 {
    if handle.is_null() || cmd_json.is_null() {
        return WICKRA_DARWIN_ERR_NULL;
    }
    let Some(cmd) = (unsafe { opt_str(cmd_json) }) else {
        return WICKRA_DARWIN_ERR_UTF8;
    };
    ...
    if !is_retry {
        ...
        let Ok(response) = catch_unwind(AssertUnwindSafe(|| {
            ...
            return WICKRA_DARWIN_ERR_PANIC;
        };
...
#[no_mangle]
pub extern "C" fn wickra_darwin_version() -> *const c_char {
...
mod tests {
    use super::{
        wickra_darwin_command, wickra_darwin_free, wickra_darwin_new, wickra_darwin_version,
        WICKRA_DARWIN_ERR_NULL,
    };
    use core::ffi::{c_char, CStr};
    use std::ffi::CString;
    use std::ptr;
    ...
    fn version_command_round_trip() {
        ...
        let handle = unsafe { wickra_darwin_new(empty.as_ptr()) };
        assert!(!handle.is_null());
        let cmd = CString::new(r#"{"cmd":"version"}"#).unwrap();
        let len = unsafe { wickra_darwin_command(handle, cmd.as_ptr(), ptr::null_mut(), 0) };
        assert!(len > 0);
        let mut buf = vec![0u8; usize::try_from(len).unwrap() + 1];
        let len2 = unsafe {
            wickra_darwin_command(
                handle,
                cmd.as_ptr(),
        ...
        unsafe { wickra_darwin_free(handle) };
    }

    #[test]
    fn evolve_without_spec_is_in_band_error() {
        let empty = CString::new("{}").unwrap();
        let handle = unsafe { wickra_darwin_new(empty.as_ptr()) };
        let cmd = CString::new(r#"{"cmd":"evolve","data":{}}"#).unwrap();
        let len = unsafe { wickra_darwin_command(handle, cmd.as_ptr(), ptr::null_mut(), 0) };
        assert!(len > 0);
        let mut buf = vec![0u8; usize::try_from(len).unwrap() + 1];
        unsafe {
    ...
    fn invalid_spec_returns_null() {
        ...
        let handle = unsafe { wickra_darwin_new(bad.as_ptr()) };
        assert!(handle.is_null());
    }
    ...
    fn null_guards() {
        let cmd = CString::new(r#"{"cmd":"version"}"#).unwrap();
        let code =
            unsafe { wickra_darwin_command(ptr::null_mut(), cmd.as_ptr(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_DARWIN_ERR_NULL);
        let empty = CString::new("{}").unwrap();
        let handle = unsafe { wickra_darwin_new(empty.as_ptr()) };
        let code = unsafe { wickra_darwin_command(handle, ptr::null(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_DARWIN_ERR_NULL);
        unsafe { wickra_darwin_free(handle) };
    }
    ...
    fn free_null_is_a_noop() {
        unsafe { wickra_darwin_free(ptr::null_mut()) };
    }

    #[test]
    fn version_is_nul_terminated() {
        let p = wickra_darwin_version();
