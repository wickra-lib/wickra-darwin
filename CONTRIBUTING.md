# Contributing to wickra-darwin

Thanks for your interest. Issues, bug reports, ideas and pull requests are all
welcome at <https://github.com/wickra-lib/wickra-darwin>. For larger changes,
open an issue first so we can agree on the approach.

## Orientation

- The core — the `EvolveSpec`, the per-symbol `SymbolState` fold, the
  `FeatureMatrix` and the `build` / `build_batch` entry points — lives in
  `crates/darwin-core`. The spec is **data, not code**: a serde struct, so
  the same feature build crosses the C ABI and WASM unchanged.
- The reference consumer is `crates/darwin-cli` (the `wickra-darwin` binary).
