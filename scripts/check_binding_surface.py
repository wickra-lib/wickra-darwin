...
make, so this reads `wickra_darwin_<name>` out of
`bindings/c/include/wickra_darwin.h` and checks each language's public surface
...
HEADER = os.path.join(ROOT, "bindings", "c", "include", "wickra_darwin.h")

# Exports that are ABI plumbing rather than a promise to callers.
ABI_ONLY = {"free"}

EXPORT = re.compile(r"\bwickra_darwin_([a-z0-9_]+)\s*\(")
...
# file only re-exports `Darwin` from the compiled module, so the methods
...
BINDINGS: dict[str, tuple[list[str], dict[str, str]]] = {
    "python": (
        ["bindings/python/src/lib.rs", "bindings/python/python/wickra_darwin/__init__.py"],
        {
            "new": r"(?m)^\s*fn new\s*\(",
    ...
    "csharp": (
        ["bindings/csharp/WickraDarwin/Darwin.cs"],
        {
            "new": r"(?m)^\s*public Darwin\s*\(",
    ...
    "go": (
        ...
        {
            "new": r"(?m)^func New\s*\(",
            "command": r"(?m)^func \([^)]*\*Darwin\) Command\s*\(",
            "version": r"(?m)^func Version\s*\(",
        },
    ),
    "java": (
        ["bindings/java/src/main/java/org/wickra/darwin/Darwin.java"],
        {
            "new": r"(?m)^\s*public Darwin\s*\(",
            "command": r"(?m)^\s*public [^\n]*\bcommand\s*\(",
            "version": r"(?m)^\s*public static [^\n]*\bversion\s*\(",
    ...
    "cpp": (
        ["bindings/c/include/wickra_darwin.hpp"],
        {
            "new": r"\bDarwin\s*\(\s*(?:const\s+)?std::string",
...
def main() -> int:
    ...
    if not exports:
        print(f"no wickra_darwin_* exports found in {HEADER}", file=sys.stderr)
        return 1
