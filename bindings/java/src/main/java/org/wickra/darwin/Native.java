...
/** Raw FFM (Panama) downcall surface for the wickra-darwin C ABI. */
final class Native {
    private Native() {}
    ...
    static final MethodHandle NEW =
            handle("wickra_darwin_new", FunctionDescriptor.of(C_PTR, C_PTR));
    static final MethodHandle FREE =
            handle("wickra_darwin_free", FunctionDescriptor.ofVoid(C_PTR));
    static final MethodHandle COMMAND =
            handle("wickra_darwin_command", FunctionDescriptor.of(C_INT, C_PTR, C_PTR, C_PTR, C_LONG));
    static final MethodHandle VERSION =
            handle("wickra_darwin_version", FunctionDescriptor.of(C_PTR));
    ...
    private static SymbolLookup loadLibrary() {
        String libFile = System.mapLibraryName("wickra_darwin");
        ...
        throw new UnsatisfiedLinkError("wickra-darwin: could not load the native library (" + libFile
    ...
    private static Path extractBundled(String libFile) {
        ...
        try (InputStream in = Native.class.getResourceAsStream(resource)) {
            ...
            Path tmp = Files.createTempFile("wickra-darwin-", "-" + libFile);
