...
/// JSON (<c>set_spec</c>, <c>evolve</c>, <c>best</c>, <c>version</c>) and read
/// back the response JSON — the same protocol as the CLI and every other binding.
/// </summary>
public sealed class Darwin : IDisposable
{
    private readonly DarwinHandle _handle;

    /// <summary>Build a search handle from a spec JSON (<c>"{}"</c> defers to a later <c>set_spec</c>).</summary>
    /// <exception cref="ArgumentException">The spec was not a valid search spec.</exception>
    public Darwin(string specJson)
    {
        IntPtr ptr = Native.wickra_darwin_new(Utf8(specJson));
        if (ptr == IntPtr.Zero)
        {
            throw new ArgumentException("wickra-darwin: invalid spec", nameof(specJson));
        }
        _handle = new DarwinHandle(ptr);
    }
    ...
    public string Command(string cmdJson)
    {
        ...
        int n = Native.wickra_darwin_command(h, cmd, null, 0);
        if (n < 0)
        {
            throw new InvalidOperationException($"wickra-darwin: command failed (code {n})");
        }
        var buf = new byte[n + 1];
        Native.wickra_darwin_command(h, cmd, buf, (nuint)buf.Length);
        return Encoding.UTF8.GetString(buf, 0, n);
    }

    /// <summary>The library version.</summary>
    public static string Version() =>
        Marshal.PtrToStringUTF8(Native.wickra_darwin_version()) ?? string.Empty;

    /// <summary>Free the native search handle.</summary>
...
internal sealed class DarwinHandle : SafeHandle
{
    public DarwinHandle(IntPtr handle)
    ...
    {
        Native.wickra_darwin_free(handle);
        return true;
    }
}
